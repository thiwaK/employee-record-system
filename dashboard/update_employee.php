...
<div class="container-fluid">
	...
	<div id="alertContainer" class="alert fade show position-fixed top-50 start-50 translate-middle" role="alert" style="display:none">
		<h4 id="alertHead" class="alert-heading m-2"></h4>
		<hr class="m-2">
		<div id="alertText" class="m-2"><div id="alertError"></div></div>
...
<script>
	...
	let alertHead = document.getElementById('alertHead');
	...
	document.getElementById('addemployee').addEventListener('submit', function(event) {
		...
		.then(data => {
			...
			if (data.response === 100) {
				alertContainer.classList.add('alert-success', 'border-success');
				alertHead.textContent = "Success";
				alertText.textContent = "New employee profile created successfully";
			} else {
				
				alertContainer.classList.add('alert-warning', 'border-warning');
				alertHead.textContent = "Error";
				alertText.innerHTML = "Failed to create new employee profile. <strong>" + data.message + "</strong>";
			}
			...
			setTimeout(() => {
				...
				alertContainer.classList.remove('alert-success', 'alert-warning', 'border-warning', 'border-success');
