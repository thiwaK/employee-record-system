<?php
    ...
    $limit = 10;
	 ...
	 //get number of rows
	if($usertype == "Admin"){
	$queryNum = $db_connect->query("SELECT COUNT(*) as postNum FROM employee WHERE  status ='Retired Employee' OR status='Transferred Employee'");
...
INNER JOIN users ON units_of_etc.unit_name=users.userunit
	WHERE employee.unit='$unitname' AND status ='Retired Employee' OR status='Transferred Employee'");
	$resultNum = $queryNum->fetch_assoc();
	$rowCount = $resultNum['postNum'];
				...
				<div class="top-bar">
					<div class="top-item">
						<form id="empFilter" method="post" action="">
							<?php if($usertype=="Admin"){?>
							<input class="filterField filterVal" type="text" placeholder="Search by Name,Designation,Unit" onkeyup="pastsearchFilter()">
						<?php }?>
						<?php if($usertype=="superuser"){?>
							<input class="filterField filterVal" type="text" placeholder="Search by Name,Designation" onkeyup="pastsearchFilter()">
						<?php }?>
						</form>
					</div>
					<div class="top-item">
